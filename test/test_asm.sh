...
DRIVER="./build/src/backend/asm_tool"
if [ ! -x $DRIVER ]; then
...
do
    ...
    $DRIVER ${srcPath} > ${asmPath}
    if [ $? -ne 0 ]; then
        echo
        echo -e "\e[31;1m[FAILED]\e[0m generate asm, see output in '${asmPath}'"
        exit 1
    fi

    # arm-linux-gnueabihf-gcc ${asmPath} ${LIB} -o ${elfPath}
    # if [ $? -ne 0 ]; then
    #     echo
    ...
    # if [ -f ${inPath} ]; then
    #     qemu-arm ${elfPath} > ${outPath} < ${inPath}
    # else
    #     qemu-arm ${elfPath} > ${outPath}
    # fi
    # result=$?
